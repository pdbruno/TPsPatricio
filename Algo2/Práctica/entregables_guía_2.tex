...
\subsubsection*{(b) Escribir formalmente el invariante de representación.}
  \Rep[$\widehat{estr}$][e]{($\forall t$: tupla $\langle nro$: nat,$contenido$: string$\rangle$, $n$: nat)\\
  ($(t \in e.inactivasNoVacias \wedge n \in e.inactivasVacias) \Rightarrow\\ (\neg vacia?(t.contenido) \wedge\\
  t.nro \neq e.seleccionada \wedge \\
  n \neq t.nro \wedge\\
  n\neq e.seleccionada))\wedge$\\
  ($\forall i$: nat)($0 \leq i \leq \#e.inactivasVacias + \#e.inactivasNoVacias + 1 \Rightarrow$\\
