...
public:
    ...
    private:
        ...
        iterator(Nodo* actual, stack<pair<Nodo*, int>> padres);

        Nodo* _actual; //es medio al dope este pero ya fue agrega claridad
        int _pos;
        stack<pair<Nodo*, int>> _padres; //la segunda componente es la posicion en el arreglo de hermanos
        void _borrar(Nodo*); //una paja que lo tengo que copiar pero we
